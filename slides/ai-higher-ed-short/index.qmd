...
bottleneck_data <- tibble(
  ...
  x = c(1, 2, 3),
  width = c(0.8, 0.3, 0.8),
  fill = c(color_tertiary, color_primary, color_secondary)
)

ggplot(bottleneck_data) +
  # Draw rectangles representing capacity
  geom_rect(
    aes(
...
geom_segment(
    aes(x = 1.45, xend = 1.82, y = 0.5, yend = 0.5),
    arrow = arrow(length = unit(0.3, "cm"), type = "closed"),
    linewidth = 1.2, color = "grey40"
  ) +
  geom_segment(
    aes(x = 2.18, xend = 2.55, y = 0.5, yend = 0.5),
    arrow = arrow(length = unit(0.3, "cm"), type = "closed"),
    linewidth = 1.2, color = "grey40"
  ) +
  # Labels above boxes
geom_text(
    aes(x = x, y = 0.82, label = component),
    size = 5, fontface = "bold", lineheight = 0.9
  ) +
  # Capacity labels inside boxes
geom_text(
    aes(x = 1, y = 0.5, label = "∞"),
    size = 8, color = "white", fontface = "bold"
  ) +
  geom_text(
    aes(x = 2, y = 0.5, label = "4±1"),
...
geom_text(
    aes(x = 2, y = 0.15, label = "DAS NADELÖHR"),
    size = 5, color = color_primary, fontface = "bold"
  ) +
  scale_fill_identity() +
...
**Arbeitsgedächtnis:** 4 ± 1 Elemente, ~30 Sekunden

:::

::: {.column width="50%"}

**Langzeitgedächtnis:** Unbegrenzt, dauerhaft
...
timeline_data <- tibble(
  era = c("1970er", "1990er", "2000er", "2020er"),
  x = c(1, 2, 3, 4),
  tech = c("Taschenrechner", "GPS", "Google", "KI"),
  effect = c("Konzeptuelles\nVerständnis ↓", "Räumliches\nGedächtnis ↓", "\"Wo\" ersetzt\n\"Was\"", "Alles oben\n+ mehr?"),
  icon = c("calculator", "location-dot", "search", "robot"),
  color = c(color_tertiary, color_secondary, color_secondary, color_primary)
)

ggplot(timeline_data) +
  # Timeline base line
  geom_segment(
    aes(x = 0.5, xend = 4.7, y = 0.5, yend = 0.5),
    linewidth = 2, color = "grey80"
  ) +
  # Timeline nodes
  geom_point(
    aes(x = x, y = 0.5, color = color),
    size = 12
  ) +
  # Progressive arrow at end
  geom_segment(
    aes(x = 4.4, xend = 4.7, y = 0.5, yend = 0.5),
    arrow = arrow(length = unit(0.4, "cm"), type = "closed"),
    linewidth = 2, color = color_primary
  ) +
  # Era labels above
  geom_text(
    aes(x = x, y = 0.72, label = era),
    size = 4.5, color = "grey50"
  ) +
  # Technology labels
  geom_text(
    aes(x = x, y = 0.5, label = c("🧮", "📍", "🔍", "🤖")),
    size = 6
  ) +
  # Effect labels below
  geom_text(
    aes(x = x, y = 0.25, label = effect, color = color),
    size = 4, fontface = "bold", lineheight = 0.9
  ) +
  scale_color_identity() +
  coord_cartesian(xlim = c(0.3, 5), ylim = c(0, 0.9)) +
  theme_void() +
...
Wenn Lernen zu leicht fühlt, findet es wahrscheinlich nicht statt.
...
::: {style="background-color: #F5E6C8; padding: 1em; border-radius: 8px; text-align: center;"}
{{< fa lightbulb >}} **Grundlagen BEVOR Werkzeuge** — und dann: schrittweise Verantwortung übergeben.
:::
